        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white p-6 shadow sm:rounded-lg">
                ...
                <form action="{{ route('pedidos.store') }}" method="POST" class="space-y-6">
                    ...
                    {{-- Dirección Origen --}}
                    <div>
                        <label for="id_direccion_origen" class="block text-sm font-medium text-gray-700">Dirección de Origen *</label>
                        <select name="id_direccion_origen" id="id_direccion_origen" class="mt-1 block w-full border rounded px-3 py-2" required>
                            <option value="">-- Selecciona --</option>
                            @foreach($direcciones as $dir)
                                <option value="{{ $dir->id_direccion }}" 
                                    @selected($val('id_direccion_origen') == $dir->id_direccion)>
                                    {{ $dir->calle_dir }} {{ $dir->numero_dir }}, {{ $dir->ciudad }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    {{-- Dirección Destino --}}
                    <div>
                        <label for="id_direccion_destino" class="block text-sm font-medium text-gray-700">Dirección de Destino *</label>
                        <select name="id_direccion_destino" id="id_direccion_destino" class="mt-1 block w-full border rounded px-3 py-2" required>
                            <option value="">-- Selecciona --</option>
                            @foreach($direcciones as $dir)
                                <option value="{{ $dir->id_direccion }}" 
                                    @selected($val('id_direccion_destino') == $dir->id_direccion)>
                                    {{ $dir->calle_dir }} {{ $dir->numero_dir }}, {{ $dir->ciudad }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="pagos[]" id="pagos" multiple class="mt-1 block w-full border rounded px-3 py-2" required>
                            @foreach($pagos as $pago)
                                <option value="{{ $pago->id_pago }}"
                                    @selected(old('pagos') && in_array($pago->id_pago, old('pagos')))>
                                    {{ $pago->metodo_de_pago }}
                                </option>
                            @endforeach
                        </select>
